...
module.exports = class AfkStatusToggle {
  ...
  getSettingsPanel() {
    ...
    urlHint.style.cssText = 'font-size:12px;color:var(--text-muted);text-align:center;';
  ...
  _createAfkButton() {
    ...
    button.style.cssText = 'background:transparent;border:none;padding:0;margin-right:4px;width:40px;height:40px;border-radius:8px;cursor:pointer;color:var(--text-normal);display:flex;align-items:center;justify-content:center;align-self:center;line-height:0;';
    button.innerHTML = `
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" width="24" height="24" fill="currentColor" style="display:block;margin:auto;">
  ...
  async _setPresence(status) {
    try {
      const module = BdApi.Webpack.getModule((m) => m && typeof m.setStatus === 'function');
      if (module?.setStatus) {
        module.setStatus(status);
        return true;
      }
      const update = BdApi.Webpack.getModule((m) => m && typeof m.updateStatus === 'function');
  ...
  async _setSelfMute(mute) {
    try {
      const voiceModule = BdApi.Webpack.getModule((m) => m && (typeof m.setSelfMute === 'function' || typeof m.setLocalMute === 'function'));
